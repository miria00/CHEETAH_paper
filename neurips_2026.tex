...
Key contributions include: (1) time-indexed variables for dynamic memory management, (2) rematerialization variables to enable cheap recomputation over costly DRAM reloads, and (3) joint tiling-fusion optimization via McCormick linearization. The hardware is not a fixed context that a schedule is solved against: MAC count, on-chip memory, on-package capacity, clock frequency, and memory technology are decision variables of the same linear program, and the cost coefficients are functions of them, so compute time is charged against the reciprocal of the MAC count, memory time against the reciprocal of the delivered bandwidth, and die area and power enter as rows over those same variables. Sizing stays continuous where the physics is continuous, while bandwidth and the discrete menus (precision, sparsity, memory technology, array aspect) are carried as one-hot selections whose coefficients are disaggregated exactly at integral selections, which keeps the program linear on each branch and avoids the relaxation leak that a raw bytes-over-bandwidth product introduces. This inner-loop precision is steered by an LLM-guided outer loop that performs causal interventions based on specific deployment profiles and Lagrangian sensitivity signals. The same formulation extends to the memory-bound decode regime of LLM serving, co-sizing on-chip memory against a bounded, quantized KV cache to shrink the decode memory wall. In less than 6 hours, \textsc{CHEETAH} discovers designs on the Pareto frontier achieving an average of $\sim22.07\times$ raw speedup across $11$ workloads, reaching up to $\sim40.1\times$ speedup on a single workload over a TPU-v3 baseline, unlocking efficiency inaccessible to decoupled sequential pipelines.\end{abstract}
...
% ---- Problem statement: objective + co-design space ----
\input{prelim}
\input{update/comparison_matrix}

% ---- Methodology: scale, solver, design choice ----
\input{3-methods}
\input{update/solver_vm}

% ---- Results: area, speedup ----
\input{5-experiments}
\input{update/batch_sensitivity}
\input{update/decode_decomp}
\input{update/tco}

% ---- Validation ----
\input{update/rtl_validation}

% ---- Discussion ----
\input{update/discussion}
\input{6-conclusion}
...
\input{lp_app}
\clearpage
\input{details_audits}
\clearpage
\input{update/deficit_sensitivity}
\clearpage
\input{solver_app}
\clearpage
\input{profiles_app}
...
\clearpage
\input{limitations}
\input{shape_aggregation}
\input{master_v3_lp}
\input{socp_conditioning}
\input{socp_provenance}
\input{coeff_provenance}
\input{cvxcluster_mapping}
% \newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
