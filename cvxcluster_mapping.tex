...
Convex relaxation $x\in[0,1]$ & The LP relaxation of the one-hots \\
...
LP and read the dual vector $\lambda=(\lambda_{\mathrm{CGM}},
...
The formulation is a linear program: bandwidth is a finite menu, so its
reciprocal is an exact one-hot expression whose LP relaxation is a lower bound,
and the compute reciprocal is bracketed by tangent and chord rows on a geometric
grid (relative half-gap $10^{-3}$), so a single LP gives the certified lower bound and
a second LP of identical sparsity, the chord LP, gives an achievable design once
every menu selector is pinned to an integral value and the pinned solution
passes the exact-lift check (with relaxed selectors the chord LP is neither
bound). Those LPs are solved with HiGHS, which returns a
certified vertex and lets us check every reported design against the constraint
set independently. The Variable-Metric-One-Armed-Rescue solver
(\cref{sec:solver_vm}) is intended for the full-resolution instances with
per-operation residency (${\sim}8.7$M rows); on the shape-aggregated instances,
where both can run, it does not meet its own convergence test within its
iteration budget (\cref{app:socp_conditioning}), so every bound reported here
is HiGHS's (\todo{measured: CORGI-vs-HiGHS agreement per regime}).
...
carries a hull disaggregation at that position, and why the product is written
over the bandwidth menu, where it is exact at integral selectors. The
multiplicative gain in \cref{tab:worked_example} and the solver machinery of
\cref{sec:solver_vm} are the same fact viewed from opposite sides:
expressiveness bought with disjunctions, and disjunctions paid for with hull
copies, a menu enumeration, and a solver that tolerates the resulting size and
conditioning.
