...
across the dynamic range rather than overfit to one point; such a stochastic or
robust objective is naturally conic and is handled directly by our
second-order-cone solver. Per-request scheduling of one realized dynamic graph is
...
% \vspace{-4pt}
\paragraph{The Joint CHEETAH-V1 LP Formulation.}
Combining tiling selection, time-indexed placement, rematerialization, and McCormick linearization yields the full V1 program. We do not display it separately: it is the special case of the V2 program \eqref{eq:v1} with the CGM one-hot $h_{\mathrm{GM}}$ pinned to the proposed chip's memory and the tile-dependent families $z^{\mathrm{act}}, z^{W}$ dropped, and both are in turn special cases of the flagship V1 program, displayed in full as \eqref{eq:v1} at the end of Section~\ref{sec:v1}.
...
Alongside the one-hot CGM selector $h_{\mathrm{GM},k}$, V3 sizes the MAC array
...
\label{eq:v3_caps}
\end{equation}
The reciprocals are convex; we represent them either \emph{exactly} by a one-hot
menu over a finite legal set of $(\mathrm{MACs},\mathrm{BW})$ points (no
approximation, ideal for the certified sub-problem), or by an outer-tangent
relaxation for a continuous LP search. Because our custom solvers also admit
second-order-cone and quadratic programs, the reciprocals may instead be written
\emph{exactly} as rotated second-order cones ($m\,v \ge 1$, $\beta\,u \ge 1$, with
no tangent gap), and the saturating utilization and cache-hit profiles as exact
concave constraints rather than one-hot grids; McCormick then remains the right
tool only for the genuinely nonconvex bilinears ($y\,p$, $u\,w$), where it is the
convex hull. This internalizes the hardware proposal the outer loop previously
made, so a single solve sizes chip \emph{and} schedule jointly under a hard
area/TDP budget.

\paragraph{Disaggregated perspective epigraph.}
The aggregate roofline row of \eqref{eq:v1},
$T_i \ge \max\!\big(\sum_c \Tmin_i(c)\,v,\; \sum_c (\Tmax_i(c){-}\mathrm{save})\,u\big)$,
...
(the source of a large relaxation gap on weight-heavy models). V3 replaces it
...
and one further McCormick family $\zeta_{i,c,k} = u\, w_{i,c,k}$ (over the box
$u \in [\underline u, \overline u]$) so that the DRAM savings scale with
bandwidth linearly. Writing $m = \mathrm{MACs}/M_{\mathrm{ref}}$ and
...
& v \;\geq\; \tfrac{2}{m_j} - \tfrac{m}{m_j^{2}}, \qquad
  u \;\geq\; \tfrac{2}{\beta_j} - \tfrac{\beta}{\beta_j^{2}}
  \quad \forall\, j \in \mathcal{J} \\[3pt]
...
& \tau_{i,c} \geq \Tmin_i(c)\, v_{i,c}, \qquad
  \tau_{i,c} \geq \Tmax_i(c)\, u_{i,c} - \sum_{k} t_i^k(c)\, \zeta_{i,c,k}, \qquad
  T_i \geq \sum_{c \in \mathcal{C}_i} \tau_{i,c} \quad \forall\, i, c \\[3pt]
& \text{Envelopes~\eqref{eq:mccormick} for } w_{i,c,k};\;
  \text{McCormick for } \zeta_{i,c,k} = u\,w_{i,c,k} \text{ on } [\underline u,\overline u]{\times}[0,1];\;
...
& T_i,\, \tau_{i,c},\, v_{i,c},\, u_{i,c} \geq 0; \quad
  y, p, p^W, r, w, z, h_{\mathrm{GM}} \in [0,1]; \quad
...
link rows inert, turns $\zeta$ into the constant multiple $u\,w$, and leaves
\eqref{eq:v1} up to the tightening of Corollary~\ref{cor:v3_hull}; this is the
precise sense in which V3 subsumes V0 and V1. When an exact certified sub-problem
is required, the tangent rows are replaced by a one-hot menu over a finite legal
set of $(\mathrm{MACs},\mathrm{BW})$ points, which introduces no approximation
and is the form used by the column-generation column-generation oracle.
...
hardware capacity, hence is a linear (or McCormick-linearizable) addition to V3
rather than an outer-loop guess. We fold them in below. Every added block is
...
the per-op dimensions and thus arithmetic intensity, and $V3$ evaluates any
$(N,K)$ without deciding them; genuinely discrete or non-convex choices (draft
...
products are linearized by the same McCormick construction as
$\zeta_{i,c,k}=u\,w_{i,c,k}$. For a mixture-of-experts layer the phase index is
...
weight, activation, and KV classes. Setting $\pi_{\mathrm{bit}}=0$ recovers
...
\Delta^{\mathrm{KV}}_{\mathrm{dram}} =
   s^{\mathrm{KV}}_{\mathrm{dec}}\, B^{\mathrm{KV}}\, d^{\mathrm{KV}},
\qquad
s^{\mathrm{KV}}_{\mathrm{dec}}\, B^{\mathrm{KV}}\, d^{\mathrm{KV}}
   \;\le\; \textstyle\sum_k 2^k h_{\mathrm{GM},k} + M\,(1-\sigma_{\mathrm{KV}}),
\label{eq:v3_kvbudget}
\end{equation}
so that a small enough budget ($\sigma_{\mathrm{KV}}=1$) lets the KV read be
served on chip and leave the DRAM term, coupling the eviction budget directly to
CGM sizing through $\lambda_{\mathrm{CGM}}$.
...
\begin{equation}
T_{\mathrm{serve}} \ge \big(\theta\,C^{\mathrm{pre}} + N\,C^{\mathrm{dec}}\big)\,v,
\qquad
T_{\mathrm{serve}} \ge \big(\theta\,M^{\mathrm{pre}} + N\,M^{\mathrm{dec}}\big)\,u,
\label{eq:v3_interleave}
\end{equation}
so $T_{\mathrm{serve}}\ge\max(\cdot,\cdot)$ over the combined stream and the optimal
...
\widehat\Delta^{\mathrm{KV}}_{\mathrm{dram}} = (1-\eta_{\mathrm{hit}})\,
   \Delta^{\mathrm{KV}}_{\mathrm{dram}},
\label{eq:v3_radix}
\end{equation}
so the solver trades cache area against the recompute and traffic it removes; the
$\eta_{\mathrm{hit}}\Delta^{\mathrm{KV}}$ product is linearized as before.
...
\underline v\,\alpha_a \le v^{a}_{i,c} \le \overline v\,\alpha_a,\quad
...
\citep{spector2024thunderkittens}.
...
in the phase where compute is idle (the bilinear $\mathrm{MACs}\cdot\phi$ term is
McCormick-linearized).
...
$\Delta^{\mathrm{moe}}_{\mathrm{dram}}+\widehat\Delta^{\mathrm{KV}}_{\mathrm{dram}}$,
...
subsumption chain
\eqref{eq:v1}$\subseteq$\eqref{eq:v1}$\subseteq$(comprehensive V3) holds by
...
$\zeta_{i,c,k}$ & op $i$, config $c$, tensor $k$ & V1 & bandwidth-scaled savings product $u\cdot w_{i,c,k}$ \\
\bottomrule
\end{tabular}
\end{table}
...
$v,\,u$            & chip scalars                 & reciprocals $1/m$, $1/\beta$ (rotated cones) \\
...
$\zeta_{i,c,k}$    & op $i$, config $c$, tensor $k$ & bandwidth-scaled savings $u\,w_{i,c,k}$ \\
\midrule
