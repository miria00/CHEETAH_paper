...
\todo{STRUCTURAL: this section currently has NO antecedent. The base serving
result at $B{=}32$ is only in App.~N (p.~91). Promote it into a new subsection
placed BEFORE this one, and lift 6.7--6.12 into their own section
``Serving-Regime Co-Design''. Also integrate the LLM outer-loop result
($44.83\times \to 73.00\times$, gate-legal, certified stop), which appears
...
\Cref{tab:perftdp} reports the result. \todo{RECONCILE with Sec.~TCO, which reports $1.15$--$1.79\times$ and a
${\sim}0.8\times$ no-break-even case. Different baselines and conventions
(serving design under the FAST TDP-ratio convention here, prefill blueprint
under the MAC-normalized convention there). State this explicitly in both
places or a reviewer reads a self-contradiction.}
...
\todo{VERIFY BEFORE SUBMISSION. Two things to check. (i) At what batch size and
on what workload is FAST's $3.7\times$ measured? Ours is a serving design at a
stated decode batch; if FAST's headline is a prefill or single-stream number the
two are not directly comparable and the caption must say so. (ii) Our margin
here ($4.45\times$ at $B{=}32$) looks SMALL relative to the raw speedup
advantage we report elsewhere. Check whether the TDP ratio is being inflated by
charging full power to a design that is bandwidth-bound and not compute-saturated,
which would understate Perf/TDP. Recompute with the measured activity factor
before quoting this as the headline efficiency comparison.}}
\label{tab:perftdp}
