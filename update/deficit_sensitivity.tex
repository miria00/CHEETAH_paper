...
Two things follow. First, at the prefill operating point of \cref{fig:speedup}
the bound can never be active: a $4096$-token prefill GEMM supplies thousands
of times more independent tiles than a $24.4\times$ chip can consume, so
$f_2$ is not a modeling artifact and no correction to the headline is owed on
this ground. We confirmed this empirically by adding the saturation bound to
the sizing program as an explicit companion row (compute charged at
$M_{\mathrm{sat}}$ in addition to the free $1/\mathrm{MACs}$ term, a valid
restriction) and re-solving the joint sizing on the real model in three regimes:
$B{=}8$, $G{=}4$; $B{=}32$, $G{=}16$ (the serving point of \cref{tab:batch});
and a decode-weighted case ($B{=}32$, context $512$, MAC domain to $25\times$)
whose unbounded optimum, $16.8\times$ TPU-v3, lies \emph{above} the decode
saturation so that the bound is provably active on the low-parallelism decode
events. In all three the chosen chip, the certified objective, and the lower
bound were identical with and without the bound to ten significant figures
(certified gaps $3\times10^{-5}$ to $4\times10^{-4}$). Where the bound is active
it does not move the optimum because decode compute is not the binding term in
a memory-bound decode; where compute binds, at prefill, the bound is never
active. Second, the concern is legitimate exactly where the
table says it is, in decode on chips beyond roughly $14\times$ TPU-v3, and it
attaches to the serving results of \cref{sec:perf_tdp} rather than to the
prefill headline; there, decode compute is rarely the binding term (decode is
memory-bound at these batches, \cref{tab:batch}), which is why the correction is
small even where it applies. Two caveats bound the claim: the geometry is the
mapper's tile model, not a placed array, and the separate question of
non-matrix work (softmax, normalization, routing) growing relative to matrix
work as the array scales is not addressed by this bound and remains an open
correction.
